...
function Controller(view, model) {
    ...
    const popupDeleteProductSuccessful = document.querySelector(
        '.product-popup__delete-successful',
    );
    const popupAddProductSuccessful = document.querySelector('.product-popup__add-successful');
    const popupAddStoreSuccessful = document.querySelector('.store-popup__add-successful');
    ...
    /**
     * Show details and information about selected store.
     *
     * @listens click
     *
    ...
    /**
     * Delete the selected product and show a pop-up window with information that the product has been successfully deleted.
     *
     * @listens click
     *
     * @param {Event} event the DOM event object.
    ...
    deleteProductConfirmBtn.addEventListener('click', (event) => {
        ...
        model
            .deleteStoreProduct(+currentProduct.id)
    ...
    this.renderProductCategory = (choosedStore) => {
        Promise.all([
            model.getProductStatusApi(+choosedStore.id, 'OK').then((products) => {
                document
                    .querySelector('.category-ok')
                    .addEventListener('click', () => view.renderTableBody(products));
            }),
            model.getProductStatusApi(+choosedStore.id, 'STORAGE').then((products) => {
                document
                    .querySelector('.category-storage')
                    .addEventListener('click', () => view.renderTableBody(products));
            }),
            model.getProductStatusApi(+choosedStore.id, 'OUT_OF_STOCK').then((products) => {
                document
                    .querySelector('.category-absent')
                    .addEventListener('click', () => view.renderTableBody(products));
            }),
            model.getProductsApi(+currentStore.id).then((products) => {
                document
                    .querySelector('.category-all')
                    .addEventListener('click', () => view.renderTableBody(products));
            }),
        ]).catch((err) => console.log(err));
    };
     ...
     * @private
     */
    deleteShopConfirmBtn.addEventListener('click', (event) => {
        event.preventDefault();
        Promise.all([
            model.deleteStoreProducts(+currentStore.id),
            model.deleteStore(+currentStore.id),
        ])
    ...
    createStoreForm.addEventListener('submit', (event) => {
        ...
        if (isValid) {
            ...
            popupAddStoreSuccessful.classList.add('is-visible');
            setTimeout(function () {
                popupAddStoreSuccessful.classList.remove('is-visible');
            }, 3000);
            event.target.reset();
        } else {
     ...
     * @private
     */
    productsPopupForm.addEventListener('submit', (event) => {
        event.preventDefault();
        let newProduct = {
        ...
        if (isValid) {
            model
                .addProductApi(+currentStore.id, newProduct)
                .then(() => model.getProductsApi(+currentStore.id))
                .then((products) => {
                    view.renderTableBody(products);
                    view.getCountOfProducts(products);
                ...
                .catch((err) => console.log(err));
            productsPopup.style.display = 'none';
            popupAddProductSuccessful.classList.add('is-visible');
            setTimeout(function () {
                popupAddProductSuccessful.classList.remove('is-visible');
            }, 3000);
            event.target.reset();
        } else {
            errorMsg.classList.add('is-visible');
    ...
    document.addEventListener('click', (event) => {
        if (event.target.className === 'product-button__edit') {
            currentProduct = event.target.closest('div');
            model
                .getProductApi(+currentProduct.id)
                .then((product) => {
                    document
                        .querySelector('.product-popup__edit-product')
                    ...
                    document.querySelector('.edit-product__company-name').value =
    ...
    editProductForm.addEventListener('submit', async function (event) {
        ...
        if (isValid) {
            model
                .editProduct(+currentProduct.id, editProduct)
                .then(() => model.getProductsApi(+currentStore.id))
                .then((products) => {
                    view.renderTableBody(products);
                })
    ...
    storesList.addEventListener('scroll', function () {
        ...
        if (
            storesList.scrollTop > scrollHeightToShow ||
            storesList.scrollTop > scrollHeightToShow
        ) {
    ...
    storeDetails.addEventListener('scroll', function () {
        ...
        if (
            storeDetails.scrollTop > scrollHeightToShowDetails ||
            storeDetails.scrollTop > scrollHeightToShowDetails
        ) {
    ...
    productLoupe.addEventListener('click', () => {
        model
            .searchProducts(ProductInput.value, +currentStore.id)
     ...
     * @private
     */
    refreshStores.addEventListener('click', () =>
        model
            .getStoresApi()
            .then((stores) => view.renderStores(stores))
            .catch((err) => console.log(err)),
    ...
    clearProductInput.addEventListener('click', () => {
        model
            .getProductsApi(+currentStore.id)
            .then((response) => {
