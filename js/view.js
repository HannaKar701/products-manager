...
function View() {
    ...
    this.renderStores = function (stores) {
        ...
        stores.forEach((store) => {
            storesListBody.insertAdjacentHTML(
                'beforeend',
                `
                <li class='stores-list__body-item ${
                    store.id === +currentStore?.id && 'js-store-active'
                }' id='${store.id}'>
    ...
    this.getCountOfProducts = function (products) {
        ...
        okProductsNum.textContent = products.filter(
            (productItem) => productItem['Status'] === 'OK',
        ).length;
        storageProductsNum.textContent = products.filter(
            (productItem) => productItem['Status'] === 'STORAGE',
        ).length;
        outOfStockProductsNum.textContent = products.filter(
            (productItem) => productItem['Status'] === 'OUT_OF_STOCK',
        ).length;
    };
